...
import {
  Activity, Boxes, ChevronDown, CircleHelp, FileKey2, KeyRound, LayoutDashboard, LogOut, Menu, ScrollText, ShieldCheck, Sparkles, X,
  type LucideIcon,
} from 'lucide-react';
...
export function AppShell() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobile ? 'mobile-open' : ''}`}>
      ...
      <nav>{nav.filter((item) => item.permission === undefined || has(item.permission)).map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} end={to === '/'} onClick={() => setMobile(false)}><Icon /><span>{label}</span><i /></NavLink>)}</nav>
    ...
    <div className="main">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          {userOpen && <div className="user-popover"><div><small>登录工作区</small><strong>{admin?.tenant?.code ?? 'platform'}</strong></div><button type="button" onClick={() => void logout()}><LogOut />退出登录</button></div>}
        </div>
      </header>
