...
Xboard 的 Blade 入口渲染于站点路由，授权脚本必须指向 `/theme/<config.name>/appgog-license/...` 公开目录；ZIP 包装目录不是部署 URL。静态入口继续按文件位置解析同一个包运行时，三个入口均先锁定后验签解锁。
