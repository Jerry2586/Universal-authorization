...
日期：2026-09-24  
状态：强制执行；v1.2.11 已完成 Phase 1，v1.2.12 已完成 Phase 2，v1.2.13 已完成 Phase 3，v1.2.14 已完成 Phase 4，v1.2.15 已完成 Phase 5，v1.2.16 已完成 Phase 6，v1.2.17 已完成 Phase 7
...
完成状态（v1.2.17）：管理菜单的安全迁移回滚请求和安装器的签名 Release 下载已拆入独立库，命令参数、数据目录和失败语义保持兼容；Docker CI 在真实首装和更新之外，会故意构建无法启动的候选镜像，确认两次诊断落盘、旧健康镜像自动恢复和业务数据不变，再执行无缓存源码修复、加密备份和空部署恢复。新增 `scripts/verify-published-release.js`，只使用 GitHub Release API 返回的七个附件地址，复用本地制品合同检查 Latest 状态、非草稿/非预发布、附件精确集合、Ed25519 签名、ZIP/RUN SHA-256、稳定引导器和包内版本/环境合同。
