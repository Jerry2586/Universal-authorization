...
export class AdminAuditQueryService {
  ...
  ): Promise<readonly AdminAuditLogRecord[]> {
    const items = await this.repository.listAuditLogs({ tenantId: context.tenantId, ...filters });
    await this.recordRead(context, 'audit-log.read', 'AUDIT_LOG', filters, items.length);
    return items;
  }
  ...
  ): Promise<readonly AdminLicenseEventRecord[]> {
    const items = await this.repository.listLicenseEvents({ tenantId: context.tenantId, ...filters });
    await this.recordRead(context, 'license-event.read', 'LICENSE_EVENT', filters, items.length);
    return items;
  }
