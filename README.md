...
v1.2.35 修复客户成品页面刷新时反复显示授权检查和串行加载问题。签名及域名检查通过后并行读取包身份、授权桥健康和服务端运行状态，提前至 DOM 就绪前执行；正常访问不显示授权安装文案，已激活管理员不再重复发现插件或注册包。保留服务端状态、安装身份和签名校验，固定 Key 与两阶段激活不变。授权桥继续使用 1.0.8。
...
生成可交付的干净 v1.2.35 安装 ZIP、版本化 `.run`、稳定 `install.sh` 和签名清单（自动排除 `.env`、数据库、密钥、旧制品和 Git 历史）：
