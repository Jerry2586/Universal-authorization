...
v1.2.32 修复 Xboard 根首页授权脚本路径：Blade 使用主题公开资源路径，静态首页与管理页继续正常加载。补充根路由、嵌套路由、无包装目录和不同 ZIP 包装目录的回归验证。沿用已激活页面仅读取公开签名状态、敏感凭据保存在服务端的边界，授权桥版本为 1.0.2。固定 Key、域名、Installation ID 和两阶段激活方式保持不变。
...
生成可交付的干净 v1.2.32 安装 ZIP、版本化 `.run`、稳定 `install.sh` 和签名清单（自动排除 `.env`、数据库、密钥、旧制品和 Git 历史）：
