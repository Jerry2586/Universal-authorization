...
v1.2.41 修复授权桥文件更新/删除时常驻响应回调影响原生后台的问题；授权桥 1.1.1 同步更新宿主守卫和后台脚本，支持“激活主题”按钮，避免把授权按钮放进删除图标区域。两阶段激活与服务端授权保持不变。故障恢复边界见 `docs/xboard-admin-recovery.md`。
...
生成可交付的干净 v1.2.41 安装 ZIP、版本化 `.run`、稳定 `install.sh` 和签名清单（自动排除 `.env`、数据库、密钥、旧制品和 Git 历史）：
