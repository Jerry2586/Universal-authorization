...
v1.2.36 修复成品主题刷新闪白：在首个外部资源之前恢复公开页面与编辑器各自的深浅色，页面底色、授权等待层与主题加载层保持一致，无加载文字或淡入淡出。读取真实主题色板，保持授权状态、签名校验及两阶段激活不变；未知主题继续兼容原有保护。
...
生成可交付的干净 v1.2.36 安装 ZIP、版本化 `.run`、稳定 `install.sh` 和签名清单（自动排除 `.env`、数据库、密钥、旧制品和 Git 历史）：
