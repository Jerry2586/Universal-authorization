...
service_state() {
  ...
  running=$(cd "$ROOT_DIR" && docker compose -p "${APPGOG_PROJECT:-appgog}" -f compose.yaml ps --status running --services 2>/dev/null | \
    awk '$0 == "license-center" || $0 == "build-center" || $0 == "build-worker" { count += 1 } END { print count + 0 }')
  case "$running" in
    3) SERVICE_STATE='正常（3/3）'; SERVICE_COLOR=$GREEN ;;
    0) SERVICE_STATE='已停止（0/3）'; SERVICE_COLOR=$RED ;;
    *) SERVICE_STATE="部分运行（$running/3）"; SERVICE_COLOR=$YELLOW ;;
  esac
}
...
configure_domains() {
  ...
  say_warn '修改域名后，还需要更新 DNS、HTTPS 证书和反向代理。已交付安装包仍指向原授权域名。'
...
logs_menu() {
  printf '%s\n' '1. 全部服务' '2. 初始化服务' '3. 授权中心' '4. 打包中心' '5. 构建 Worker' '0. 返回'
  tty_read '请选择日志来源：'
  case "$REPLY_VALUE" in
    1) service=all ;; 2) service=initialize ;; 3) service=license-center ;;
    4) service=build-center ;; 5) service=build-worker ;; 0|'') return 0 ;;
...
restore_menu() {
  printf '可用备份：\n'
  find "$ROOT_DIR/backups" -maxdepth 1 -type f -name 'appgog-*.tar.gz' -print 2>/dev/null | sort -r | head -n 10 || true
...
advanced_menu() {
  while :; do
    header
    printf '%s\n' '高级工具' '  1. 系统诊断' '  2. Docker 磁盘占用' '  3. 检查 Compose 配置' '  4. 显示安装目录' '  5. 查看帮助' '  0. 返回主菜单'
    tty_read '请选择：'
    case "$REPLY_VALUE" in
      1) run_docker doctor; pause_menu ;;
      2) docker system df; pause_menu ;;
      3) (cd "$ROOT_DIR" && docker compose -p "${APPGOG_PROJECT:-appgog}" -f compose.yaml config --quiet) && say_ok 'Compose 配置有效'; pause_menu ;;
      4) printf '%s\n' "$ROOT_DIR"; pause_menu ;;
      5) usage; pause_menu ;;
      0|'') return 0 ;;
...
case "${1:-menu}" in
  menu) main_menu ;;
  install|status|start|stop|restart|update|backup|credentials|doctor) run_docker "$1" ;;
