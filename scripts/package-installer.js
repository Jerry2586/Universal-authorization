...
export function createInstaller(archive, releaseName) {
...
missing=false
for tool in unzip base64 sha256sum; do command -v "$tool" >/dev/null 2>&1 || missing=true; done
if [ "$missing" = true ]; then
  echo '正在补齐自解压所需工具...'
  if command -v apt-get >/dev/null 2>&1; then
    apt-get update
    DEBIAN_FRONTEND=noninteractive apt-get install -y unzip coreutils
  elif command -v dnf >/dev/null 2>&1; then dnf install -y unzip coreutils
  elif command -v yum >/dev/null 2>&1; then yum install -y unzip coreutils
